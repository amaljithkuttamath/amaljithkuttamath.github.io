...
// The per-turn session core (createSession + its closures) stays in this file;
// the pure, session-independent pieces were split out by responsibility and are
// imported here for internal use AND re-exported below so every existing
// `import { ... } from './agent'` keeps working.
...
// Re-export facade for the split-out modules (preserves the public API).
export { buildSystemPrompt, buildSubAgentPrompt } from './prompts';
export { needsPlan, countCountryMentions, parsePlanBrief, matchStepToEvent } from './planner';
export type { PlanStep, InsightBrief } from './planner';
export { parseVerifierVerdict } from './verifier';
export type { VerifyStatus, VerificationVerdict, ParsedVerdict } from './verifier';
