...
export function renderVerification(tb: TurnBlock, v: AgentOutput['verification']) {
  ...
  const text = v?.engine === 'jev' ? (v.status === 'verified' ? 'Jev checks passed · model judgment, not proof of correctness' : v.status === 'unavailable' ? 'Jev review unavailable · inspect the receipt for details' : 'Jev flagged a review gap · ' + (v.issues[0] || 'the finding could not be confirmed')) : verificationCueText(v);
