...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_POST(self):
        ...
        if self.path != '/api/jev/run': return self.send_json(404, {'error': 'Not found.'})
        try:
            size = int(self.headers.get('Content-Length', '0'))
            if not 0 < size <= 8192: return self.send_json(413, {'error': 'Request too large.'})
            body = json.loads(self.rfile.read(size)); prompt = body.get('prompt')
